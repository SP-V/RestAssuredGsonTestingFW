...
public class AllTransactionsByDecDate extends TestBase {

  @Test public void allTransactionsByDecDate() {

    Response response = given().auth().preemptive().basic(user, pass).when()
        .get(baseUrl.concat("?").concat("orderBy=").concat(orderByCriterias.get(0)));
