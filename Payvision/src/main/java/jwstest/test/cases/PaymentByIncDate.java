...
public class PaymentByIncDate extends TestBase {
  @Test public void paymentByIncDate() {

    for (String currency : currencyCodes) {

      Response response = given().auth().preemptive().basic(user, pass).when().get(
          baseUrl.concat("?").concat("action=").concat(actions.get(0)).concat("&")
              .concat("currencyCode=").concat(currency).concat("&orderBy=-date"));
